...
function ready() {
    ...
    document.getElementsByClassName("shack")[0].addEventListener('click', function(){
        if (localStorage.getItem("spot" + localStorage.getItem("selected")) == "key"){
            window.location.href = "../shack/shack.html";
        }
    });

    var shellBut = document.getElementsByClassName("shell-but")[0];
    if (localStorage.getItem("has-shell") == "true") {
        take(shellBut, "shell");
    } else {
        shellBut.addEventListener('click', function tk() {
            shellBut.removeEventListener('click', tk);
            take(shellBut, "shell");
        });
    }
}
...
function take(button, name) {
    if (localStorage.getItem("free") < 9 || init == "true") {
        document.getElementsByClassName("fore-art")[0].classList.add("invis");
...
function put(button, name) {
    if (localStorage.getItem("spot" + localStorage.getItem("selected")) == name) {
        document.getElementsByClassName("fore-art")[0].classList.remove("invis");
