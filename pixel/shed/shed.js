...
function ready() {
    ...
    var mapBut = document.getElementsByClassName("shed-map-but")[0];
    if (localStorage.getItem("has-shed-map") == "true") {
        take(mapBut, "shed-map");
    } else {
    ...
    var cseedsBut = document.getElementsByClassName("cornflower-seeds-but")[0];
    if (localStorage.getItem("has-cornflower-seeds") == "true") {
        take(cseedsBut, "cornflower-seeds");
    } else {
        cseedsBut.addEventListener('click', function tk() {
            cseedsBut.removeEventListener('click', tk);
            take(cseedsBut, "cornflower-seeds");
        });
    }

    var pseedsBut = document.getElementsByClassName("poppy-seeds-but")[0];
    if (localStorage.getItem("has-poppy-seeds") == "true") {
        take(pseedsBut, "poppy-seeds");
    } else {
        pseedsBut.addEventListener('click', function tk() {
            pseedsBut.removeEventListener('click', tk);
            take(pseedsBut, "poppy-seeds");
        });
    }
}
